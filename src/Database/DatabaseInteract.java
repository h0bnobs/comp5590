...
//---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    public List<HashMap<String, Object>> getAllPreviousAppointments(String pid) {
        ...
        try {
            ...
            ResultSet results = preparedStatement.executeQuery();
            analyseResults(appointments, results);
        } catch (Exception e) {
            System.out.println(e);
        }
    ...
    private void analyseResults(List<HashMap<String, Object>> appointments, ResultSet results) throws SQLException {
        while (results.next()) {
...
//---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    public List<HashMap<String, Object>> getFutureAppointmentsByMonth(String pid, String did, String yearAndMonth) {
        ...
        try {
            ...
            appointmentPrepareStatementLogic(pid, did, yearAndMonth, appointments, currentDateTime, query);
        } catch (Exception e) {
            System.out.println(e);
        }
        return appointments;
    ...
    private void appointmentPrepareStatementLogic(String pid, String did, String yearAndMonth, List<HashMap<String, Object>> appointments, String currentDateTime, String query) throws SQLException {
        PreparedStatement preparedStatement = connection.prepareStatement(query);
        preparedStatement.setString(1, pid);
        preparedStatement.setString(2, did);
...
//---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    public List<HashMap<String, Object>> getFutureAppointmentsByFullDate(String pid, String did, String yearMonthDate) {
        ...
        try {
            ...
            appointmentPrepareStatementLogic(pid, did, yearMonthDate, appointments, currentDateTime, query);
        } catch (Exception e) {
            System.out.println(e);
        }
        return appointments;
    ...
    public void addMessage(HashMap<String, Object> userInformation, String message, String pid) {
        try {
            ...
            if (!resultSet.next()) {
                ...
                //if there is already a message that matches, add dupe.
                int count = 1;
                String existingMessage = resultSet.getString("message_content");

                // Extract the current count if it exists
                Pattern pattern = Pattern.compile("\\((\\d+)\\)$");
                Matcher matcher = pattern.matcher(existingMessage);

                if (matcher.find()) {
                    count = Integer.parseInt(matcher.group(1)) + 1;
                    // Remove the existing count from message to prepare for update
                    existingMessage = existingMessage.replaceAll("\\(\\d+\\)$", "").trim();
                }

                // Append the new count to the message content
                String updatedMessage = existingMessage + " (" + count + ")";

                // Update the message in the database
                String updateQuery = "UPDATE messages SET message_content = ? WHERE pid = ? AND message_content = ?";
                PreparedStatement updateStatement = connection.prepareStatement(updateQuery);
                updateStatement.setString(1, updatedMessage);
...
//---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    public List<String> getUserMessages(String pid) {
        ...
        try {
            String query = "SELECT message_content FROM messages WHERE pid = ? ORDER BY message_id";
            PreparedStatement preparedStatement = connection.prepareStatement(query);
            preparedStatement.setString(1, pid);
